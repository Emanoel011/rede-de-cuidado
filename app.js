...
async function doRegister(e) {
    ...
    await supabase.from('registros').insert([{ chave: email, valor: { nome_completo: name } }]);
...
async function doRecover(e) {
    ...
    if (error) showMsg(error.message, 'err');
    else showMsg('Link enviado para o seu e-mail!', 'ok');
}
...
async function loadTable() {
    ...
    if (error) return showMsg('Erro ao carregar', 'err');
    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="3" class="text-center py-12 text-gray-400">Nenhum registro</td></tr>`;
        return;
    }
    ...
    data.forEach(row => {
        ...
        tr.innerHTML = `
            <td class="p-4">${row.chave}</td>
            <td class="p-4"><pre class="text-xs">${JSON.stringify(row.valor, null, 2)}</pre></td>
...
async function insertNewRow() {
    ...
    if (!chave) return;
    const valorStr = prompt("Digite o JSON:", "{}");
    try {
